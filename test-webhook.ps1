# Script de diagnóstico para probar el webhook de Google Apps Script
# Este script hace pruebas progresivas para identificar dónde está el problema

Write-Host "================================================" -ForegroundColor Cyan
Write-Host "   DIAGNÓSTICO DEL WEBHOOK - APPS SCRIPT" -ForegroundColor Cyan
Write-Host "================================================" -ForegroundColor Cyan
Write-Host ""
...
function Test-Webhook {
    ...
    try {
        ...
        Write-Host "      Deberías ver líneas que empiecen con:" -ForegroundColor Cyan
...
function Test-CreateMovement {
    ...
    Write-Host ""
    Write-Host "📝 Paso 3: Creando una operación de prueba..." -ForegroundColor Yellow
    ...
    try {
        Write-Host "   📡 Enviando operación..." -ForegroundColor Gray
        $response = Invoke-RestMethod -Uri "$baseUrl/api/sheets/movements" -Method POST -Headers $headers -Body $movementData
        
        Write-Host "   ✅ Operación creada exitosamente" -ForegroundColor Green
        ...
        Write-Host "      1. Si se llamó al webhook automáticamente" -ForegroundColor White
        Write-Host "      2. La respuesta del Web App de Google" -ForegroundColor White
        
        return $true
    }
    catch {
        Write-Host "   ❌ Error al crear operación: $_" -ForegroundColor Red
        return $false
    }
}
...
if (-not $token) {
    ...
    Write-Host "   1. Que el backend esté corriendo en $baseUrl" -ForegroundColor Yellow
...
Write-Host "================================================" -ForegroundColor Cyan
Write-Host ""
Write-Host "✅ Token obtenido: SÍ" -ForegroundColor $(if ($token) { "Green" } else { "Red" })
Write-Host "✅ Webhook manual: $(if ($webhookSuccess) { 'SÍ' } else { 'NO' })" -ForegroundColor $(if ($webhookSuccess) { "Green" } else { "Red" })
Write-Host "✅ Operación creada: $(if ($movementSuccess) { 'SÍ' } else { 'NO' })" -ForegroundColor $(if ($movementSuccess) { "Green" } else { "Red" })
Write-Host ""
Write-Host "📋 PRÓXIMOS PASOS:" -ForegroundColor Cyan
Write-Host ""
Write-Host "1. Revisa los logs del backend en tu terminal" -ForegroundColor Yellow
Write-Host "   Busca las secciones marcadas con 🚀 y ✅/❌" -ForegroundColor Gray
Write-Host ""
Write-Host "2. Si ves '❌ WEB APP CALL FAILED', copia el error y envíamelo" -ForegroundColor Yellow
Write-Host ""
Write-Host "3. Si ves '✅ WEB APP RESPONSE SUCCESS', pero las filas no se procesan:" -ForegroundColor Yellow
Write-Host "   - El problema está en el código de Google Apps Script" -ForegroundColor Gray
Write-Host "   - Necesitarás que alguien acceda al Apps Script y revise los logs" -ForegroundColor Gray
Write-Host ""
Write-Host "4. Ve a FORM_INPUT en Google Sheets y verifica:" -ForegroundColor Yellow
Write-Host "   - ¿Se creó la fila 'Test Diagnostico'?" -ForegroundColor Gray
